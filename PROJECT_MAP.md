...
> آخر تحديث: 2026-08-03 (Milestones 1-11 ✅ + تحديثات نموذج العمل 2026-08)
...
→ Student Dashboard (7 tabs)
  ├── 📖 قرآن: طلب تحفيظ → structured-request → عرض الاقتراح (سعر AI + مدرس مرشّح) ← المدرس يقرر السعر ← أول جلسة مجانية ← دفع لاحقاً ← متابعة تقدم
  ├── 🎓 جامعي: رفع PDF → AI تحليل + اقتراح سعر → ترشيح مدرس ← المدرس يؤكد/يعدّل السعر ← أول جلسة مجانية ← دفع ← جلسة
  ├── 📐 ثانوي: نفس جامعي (أسعار مختلفة)
  ├── 📚 إعدادي: نفس جامعي (أسعار مختلفة)
  ├── ✏️ ابتدائي: نفس جامعي (أسعار مختلفة)
  ├── 🧸 حضانة: structured-request → عرض الاقتراح ← المدرس يقرر السعر ← متابعة تقدم
  └── 🌐 لغات: structured-request → عرض الاقتراح ← المدرس يقرر السعر ← متابعة تقدم
```

### رحلة المستخدم — Tutor

```
Landing Page → Register (اختيار مستوى واحد + لغات لو لغات + إنستاباي/فودافون كاش)
→ قيد مراجعة الإدارة → الاعتماد اليدوي (is_approved)
→ Tutor Dashboard (يرى فقط طلبات مستواه)
  ├── طلبات مقترحة عليك (AI-First) ← قبول/تعديل السعر/رفض (PUT /offers/<id>/respond/)
  ├── سجل عروضي (pending/accepted/rejected)
  ├── جلساتي ← تسجيل تقدم (حسب النوع: juz/unit/cefr) ← أول جلسة مجانية is_trial
  └── الأرباح: 85% تُجمّع شهرياً وتُصرف عبر إنستاباي/فودافون كاش (Payout)
```
...
| 12 | تحديثات نموذج العمل: المدرس يقرر السعر (respond) + أول جلسة مجانية is_trial + مصر بالجنيه + إنستاباي/فودافون كاش شهرياً (Payout) + اعتماد يدوي + شكاوى/حظر + القانون المصري | ✅ (46 اختباراً + Build ناجح) |
